...
$(document).ready(function () {
    ...
    let date = new Date();
    let getOllComment = document.querySelectorAll(".preland-comment-data-generate");

    let time = new Array();

    time[0] = new Date(date.getTime() - (300 * 1000));

    for (let i = 1; i < getOllComment.length; i++) {
        time[i] = new Date(time[i - 1].getTime() - Math.floor(Math.random() * (14400 * 1000)));
    }

    for (let i = 0; i < getOllComment.length; i++) {
        $(".preland-comment-data-generate").eq(i).html(getNormal(time[i].getDate()) + "." + monthFull[time[i].getMonth()] + "." + time[i].getFullYear());
    }

    let dateHeader = new Date(time[time.length - 1].getTime() - (86400 * 35 * 1000));
    $(".header-date").html('<i class="fa fa-calendar" aria-hidden="true"></i>&nbsp;' + getNormal(dateHeader.getDate() + " " + monthFull[dateHeader.getMonth()] + " " + dateHeader.getFullYear()));

    let dateTomorrow = new Date(date.getTime() + (86400 * 1000));
    $(".preland-offer-date-end").html(getNormal('<i class="fa fa-calendar" aria-hidden="true"></i>&nbsp;' + dateTomorrow.getDate() + "&nbsp;" + monthFull[dateTomorrow.getMonth()] + "&nbsp;" + dateTomorrow.getFullYear()));
});
